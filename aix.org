...
- =chuser attribute=value username= ::
	Set the soft or hard limits for a user.
...
- =trus= ::
	Produces a trace of the system calls it performs, the signals it receives and the machine faults it incurs. By default, user-level functions are not traced.
...
- =mklv/extendlv -a= :: = PP allocation policy [ =-am= = middle; =-ac= = cemter; =-ac= = center; =-ae= = edge; =-aie= = inner edge; =-aim= = inner middle ]
